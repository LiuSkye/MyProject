...
{
#if VALGRIND //valgrind: 内存检测工具 可以检测内存泄漏、文件描述符打开状态
...
int sockets::accept(int sockfd, struct sockaddr_in6 *addr)
{
    ...
    if (connfd < 0)
    {
        int savedErrno = errno; // 保存errno再判断，因为errno会被改变
...
// 与read不同之处在于：接收的数据可以填充到多个缓冲区中，iovec指定多块缓冲区
